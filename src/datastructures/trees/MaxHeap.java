...
public class MaxHeap<T extends Comparable<T>> implements IDataStructure<T> {
	...
	@Override
	public String info() {
		String result = "A Max Heap is a binary tree based data structure "
				+ "where each parent node is greater than or equal to its child nodes. "
				+ "This ensures that the maximum element is always stored at the root of the heap.\n\n";

		result += "The Max Heap is commonly implemented using arrays (or ArrayLists in Java), "
				+ "where for a node at index i:\n"
				+ "- The left child is at index 2*i + 1\n"
				+ "- The right child is at index 2*i + 2\n"
				+ "- The parent is at index (i - 1) / 2\n\n";

		result += "Operations in a Max Heap:\n"
				+ "- Insertion (push): The element is added at the end and then 'bubbled up' "
				+ "to maintain the heap property.\n"
				+ "- Deletion (pop): The root element (maximum) is removed, and the last element "
				+ "is moved to the root, followed by a 'bubble down' to restore the heap property.\n"
				+ "- Peek: Retrieves the maximum element (root) without removing it.\n\n";

		result += "'n' is the number of elements in the heap.\n\n";

		result += "Access Time Complexity: O(1) for max (root)\n";
